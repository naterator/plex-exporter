...
func (l *retryTestListener) Listen(context.Context, log.Logger) error {
	l.calls++
	if l.calls == 3 {
		l.cancel()
	}
	return errors.New("synthetic websocket failure")
}

func TestListenWithRetryReconnectsUntilCanceled(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	listener := &retryTestListener{cancel: cancel}

	if err := listenWithRetry(ctx, listener, log.NewNopLogger(), time.Millisecond, 2*time.Millisecond); err != nil {
...
type fatalRetryTestListener struct {
	calls int
}

func (l *fatalRetryTestListener) Listen(context.Context, log.Logger) error {
	l.calls++
	return plex.ErrServerClosed
}

func TestListenWithRetryReturnsPermanentLifecycleErrors(t *testing.T) {
	listener := &fatalRetryTestListener{}
	err := listenWithRetry(context.Background(), listener, log.NewNopLogger(), time.Hour, time.Hour)
	if !errors.Is(err, plex.ErrServerClosed) {
		t.Fatalf("listenWithRetry error = %v, want ErrServerClosed", err)
	}
	if listener.calls != 1 {

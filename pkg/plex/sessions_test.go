...
func TestUpdateStopsSessionWithoutMedia(t *testing.T) {
	s := &sessions{sessions: map[string]session{}, server: &Server{Name: "fake-srv", ID: "fake-id"}}
	s.sessions["partial"] = session{
		session:     ttPlex.Metadata{SessionKey: "partial"},
		state:       statePlaying,
		playStarted: time.Now().Add(-time.Second),
	}

	// Plex websocket events can arrive before their Media array is populated.
	// Stopping such a session must still record play time without indexing an
	// absent media item.
	s.Update("partial", stateStopped, nil, nil)

	if got := s.sessions["partial"].prevPlayedTime; got <= 0 {
		t.Fatalf("expected partial session play time to be retained, got %v", got)
	}
	if s.totalEstimatedTransmittedKBits != 0 {
		t.Fatalf("expected no bitrate estimate without media, got %v", s.totalEstimatedTransmittedKBits)
	}
}
...
func TestSessionTranscodeMappingTransfersEarlyUpdate(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	store := NewSessions(ctx, &Server{})

	store.SetTranscodeType("/transcode/sessions/early-transcode", "video")
	store.SetSubtitleAction("/transcode/sessions/early-transcode", "burn")
	metadata := ttPlex.Metadata{SessionKey: "play-session"}
	store.Update("play-session", statePlaying, &metadata, &ttPlex.Metadata{RatingKey: "movie-1"})
	store.SetSessionTranscodeMapping("play-session", "early-transcode")

	store.mtx.Lock()
	defer store.mtx.Unlock()
	got := store.sessions["play-session"]
	if got.transcodeType != "video" || got.subtitleAction != "burn" {
		t.Fatalf("early transcode attributes were not transferred: %+v", got)
	}
	if _, exists := store.sessions["/transcode/sessions/early-transcode"]; exists {
		t.Fatal("temporary transcode session was not removed after transfer")
	}
}

func TestSessionTranscodeMappingMatchesRawAndFullPathForms(t *testing.T) {
	store := &sessions{
		sessions: map[string]session{
			"play-session": {session: ttPlex.Metadata{SessionKey: "play-session"}},
		},
		sessionTranscodeMap: make(map[string]string),
		server:              &Server{},
	}
	store.SetSessionTranscodeMapping("play-session", "/transcode/sessions/transcode-id")
	if !store.TrySetTranscodeType("transcode-id", "audio") {
		t.Fatal("raw transcode ID did not match full-path mapping")
	}
	if !store.TrySetSubtitleAction("transcode-id", "copy") {
		t.Fatal("raw transcode ID did not match full-path subtitle mapping")
	}
}

func TestDirectPlayMappingClearsStaleTranscodeAttributes(t *testing.T) {
	store := &sessions{
		sessions: map[string]session{
			"play-session": {
				session:        ttPlex.Metadata{SessionKey: "play-session"},
				transcodeType:  "both",
				subtitleAction: "burn",
			},
		},
		sessionTranscodeMap: map[string]string{"play-session": "old-transcode"},
		server:              &Server{},
	}

	store.SetSessionTranscodeMapping("play-session", "")
	got := store.sessions["play-session"]
	if got.transcodeType != "" || got.subtitleAction != "" {
		t.Fatalf("direct play retained stale transcode attributes: %+v", got)
	}
	if _, exists := store.sessionTranscodeMap["play-session"]; exists {
		t.Fatal("direct play retained stale transcode mapping")
	}
}

func TestEmptySessionIdentifiersAreIgnored(t *testing.T) {
	store := &sessions{
		sessions:            make(map[string]session),
		sessionTranscodeMap: make(map[string]string),
		server:              &Server{},
	}
	metadata := ttPlex.Metadata{SessionKey: "should-not-be-used"}
	store.Update(" ", statePlaying, &metadata, &metadata)
	store.SetSessionTranscodeMapping("", "transcode")
	store.SetTranscodeType("", "video")
	store.SetTranscodeType("/transcode/sessions/", "video")
	store.SetSubtitleAction("", "burn")
	store.SetSubtitleAction("/transcode/sessions/", "burn")
	if store.TrySetTranscodeType("", "video") || store.TrySetTranscodeType("/transcode/sessions/", "video") {
		t.Fatal("empty transcode identifier unexpectedly matched")
	}
	if store.TrySetSubtitleAction("", "burn") || store.TrySetSubtitleAction("/transcode/sessions/", "burn") {
		t.Fatal("empty subtitle identifier unexpectedly matched")
	}
	if len(store.sessions) != 0 || len(store.sessionTranscodeMap) != 0 {
		t.Fatalf("empty identifiers mutated state: sessions=%v mappings=%v", store.sessions, store.sessionTranscodeMap)
	}
}

func TestCollectKeepsPlayerLabelsWhenSessionMediaIsMissing(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	server := &Server{Name: "server", ID: "server-id"}
	store := NewSessions(ctx, server)
	playSession := ttPlex.Metadata{
		SessionKey: "partial-session",
		Player:     ttPlex.Player{Device: "Living Room", Product: "Plex TV"},
		User:       ttPlex.User{Title: "viewer"},
	}
	media := ttPlex.Metadata{RatingKey: "movie-1", Type: "movie", Title: "Movie"}
	store.Update("partial-session", statePlaying, &playSession, &media)

	metricsChannel := make(chan prometheus.Metric, 10)
	store.Collect(metricsChannel)
	close(metricsChannel)
	for metric := range metricsChannel {
		_, labels, err := readMetric(metric)
		if err != nil {
			t.Fatalf("readMetric: %v", err)
		}
		if labels["session"] != "partial-session" {
			continue
		}
		if labels["device"] != "Living Room" || labels["device_type"] != "Plex TV" || labels["user"] != "viewer" {
			t.Fatalf("player labels were dropped: %v", labels)
		}
		return
	}
	t.Fatal("partial session metrics were not collected")
}
...
func TestSessionsCloseStopsWorkerAndIsIdempotent(t *testing.T) {
	...
	case <-store.done:
	default:
		t.Fatal("session worker is still running after Close")
	}
}

func TestSyncCurrentReconcilesWithoutOverwritingNewerEvents(t *testing.T) {
	store := NewSessions(context.Background(), &Server{})
	defer store.Close()
	observedAt := time.Now()
	startedAt := observedAt.Add(-time.Minute)
	store.sessions["missing-stale"] = session{
		session:     ttPlex.Metadata{SessionKey: "missing-stale"},
		state:       statePlaying,
		playStarted: startedAt,
		lastUpdate:  observedAt.Add(-time.Second),
	}
	store.sessions["missing-newer"] = session{
		session:     ttPlex.Metadata{SessionKey: "missing-newer"},
		state:       statePlaying,
		playStarted: startedAt,
		lastUpdate:  observedAt.Add(time.Nanosecond),
	}
	store.sessions["snapshot-stopped-newer"] = session{
		session:     ttPlex.Metadata{SessionKey: "snapshot-stopped-newer"},
		state:       stateStopped,
		playStarted: startedAt,
		lastUpdate:  observedAt.Add(time.Nanosecond),
	}

	store.syncCurrent([]ttPlex.Metadata{
		{SessionKey: "present-paused", Player: ttPlex.Player{State: "paused"}},
		{SessionKey: "snapshot-stopped-newer", Player: ttPlex.Player{State: "playing"}},
	}, observedAt)

	if got := store.sessions["missing-stale"].state; got != stateStopped {
		t.Fatalf("stale missing session state = %q, want stopped", got)
	}
	if got := store.sessions["missing-newer"].state; got != statePlaying {
		t.Fatalf("newer missing session state = %q, want playing", got)
	}
	if got := store.sessions["snapshot-stopped-newer"].state; got != stateStopped {
		t.Fatalf("newer stop was overwritten by snapshot: %q", got)
	}
	paused := store.sessions["present-paused"]
	if paused.state != statePaused || paused.playStarted.IsZero() {
		t.Fatalf("paused snapshot was not seeded: %+v", paused)
	}
}

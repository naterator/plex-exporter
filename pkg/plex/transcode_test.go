...
func TestSubtitleActionRequiresSubtitleEvidence(t *testing.T) {
	tests := []struct {
		name      string
		session   ttPlex.TranscodeSession
		want      string
		...
		{
			name:      "explicit burn",
			session:   ttPlex.TranscodeSession{SubtitleDecision: "burn-in"},
			want:      "burn",
			wantKnown: true,
		},
		{
			name:      "explicit transcode",
			session:   ttPlex.TranscodeSession{SubtitleDecision: "transcoding"},
			want:      "transcode",
			wantKnown: true,
		},
		{
			name:      "srt copy",
			session:   ttPlex.TranscodeSession{Container: "srt"},
			want:      "copy",
			wantKnown: true,
		},
		{
			name:      "video transcode alone is not subtitle burn",
			session:   ttPlex.TranscodeSession{VideoDecision: "transcode"},
			want:      "none",
			wantKnown: false,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, known := subtitleAction(test.session)
			if got != test.want || known != test.wantKnown {
				t.Fatalf("subtitleAction() = (%q, %t), want (%q, %t)", got, known, test.want, test.wantKnown)
			}
		})
	}
}

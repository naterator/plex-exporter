...
func TestPlayDefaultsTranscodeAndSubtitleToNone(t *testing.T) {
	m := Play(1.0, "plex", "srv", "id", "libtype", "lib", "libid", "movie", "Title", "Child", "Grand", "DirectPlay", "1080", "1080", "8000", "Device", "DevType", "user", "sess", "", "")
	...
	if v := metricLabelValue(m, "library_type"); v != "libtype" {
		t.Fatalf("expected library_type=libtype, got %q", v)
	}
	if v := metricLabelValue(m, "library"); v != "lib" {
		t.Fatalf("expected library=lib, got %q", v)
	}
	if v := metricLabelValue(m, "library_id"); v != "libid" {
		t.Fatalf("expected library_id=libid, got %q", v)
	}
}

func TestPlayEmitsProvidedTranscodeAndSubtitle(t *testing.T) {
	m := Play(1.0, "plex", "srv", "id", "libtype", "lib", "libid", "movie", "Title", "Child", "Grand", "transcode", "1080", "1080", "8000", "Device", "DevType", "user", "sess", "audio", "copy")
...
func TestPlayDurationDefaultsTranscodeAndSubtitleToNone(t *testing.T) {
	m := PlayDuration(5.0, "plex", "srv", "id", "libtype", "lib", "libid", "movie", "Title", "Child", "Grand", "DirectPlay", "1080", "1080", "8000", "Device", "DevType", "user", "sess", "", "")
	if v := metricLabelValue(m, "transcode_type"); v != "none" {
		t.Fatalf("expected transcode_type=none, got %q", v)
	}
	if v := metricLabelValue(m, "subtitle_action"); v != "none" {
